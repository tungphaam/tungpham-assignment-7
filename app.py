...
def hypothesis_test():
    ...
    if test_type == ">":
        ...
        p_value = 0 # Default value

    # TODO 11: If p_value is very small (e.g., <= 0.0001), set fun_message to a fun message
    fun_message = None
    if isinstance(p_value, float) and p_value <= 0.0001:
        fun_message = "Wow! You've encountered a rare event with a very small p-value!"


    # TODO 12: Plot histogram of simulated statistics
    plt.figure(figsize=(8, 6))
    plt.hist(simulated_stats, bins=20, alpha=0.6, color='skyblue', label="Simulated Statistics")
    plt.axvline(observed_stat, color="red", linestyle="--", linewidth=2, label=f"Observed {parameter.capitalize()}: {observed_stat:.4f}")
    plt.axvline(hypothesized_value, color="blue", linestyle="--", linewidth=2, label=f"Hypothesized {parameter.capitalize()} (H0): {hypothesized_value:.4f}")
...
def confidence_interval():
    ...
    plt.axvline(mean_estimate, color='blue', linestyle="--", label=f"Mean Estimate: {mean_estimate:.4f}")
    plt.plot([ci_lower, ci_upper], [0, 0], color="green", linestyle="-", linewidth=2, label=f"{confidence_level}% Confidence Interval")
    plt.axvline(true_param, color="green", linestyle="--", label=f"True {parameter.capitalize()}: {true_param:.4f}")
